...
| 2026-09 | Active workout | Landing on the next lift (e.g. OHP) showed the previous lift's weight/reps in the steppers until `useEffect` seeded, so set 1 looked pre-filled; a follow-up Log Set after no-rest advance could also write that load onto the new `workout_exercise_id` | `useLayoutEffect` seed + clear watch drafts on exercise change; `commitSetLog` reads live weight/reps refs; `resolveExerciseInputSeed` unit tests |

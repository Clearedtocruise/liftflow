...
type MealMacroFields = {
  ...
  /** 'log' = the user typed/logged this meal themselves; 'plan' = generated by the meal planner. */
  origin?: string;
};
 ...
 *
 * This must stay narrowly scoped to that specific mis-parse pattern (a dense-fat
 * ingredient like peanut butter combined with other items in one free-text line).
 * A bare "stored calories are a lot higher than our tiny local food catalog would
 * guess" ratio check is not safe on its own — anything not in that ~30-item catalog
 * (a branded mass-gainer shake, a restaurant meal, a protein bar) legitimately has
 * more calories than the naive per-ingredient estimate, and blindly overriding a
 * genuinely logged/measured meal with the low catalog guess silently rewrites the
 * user's real numbers (the reported "it edited to whatever it wanted" bug).
 */
...
export function resolveMealMacros(meal: MealMacroFields): MealMacros {
  // This heuristic exists to catch a *meal-plan generator* bug (a bogus daily-calorie total
  // leaking into one slot, e.g. a weight-unit mixup — see looksLikeInflatedPlanMacros). It must
  // never run against a meal the user logged themselves: a lifter who genuinely ate/drank a
  // 600 cal pre-workout shake (over the 450 cal "pre_workout" sanity cap) had their explicit
  // calories and macros silently rescaled to ~288 cal, which looked like the app "editing" their
  // number to whatever it wanted.
  if (meal.origin !== 'log' && looksLikeInflatedPlanMacros(meal)) {
    return correctInflatedPlanMacros(meal);
  }

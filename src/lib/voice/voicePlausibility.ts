/**
 * Plausibility rules for parsed voice sets.
 *
 * A speech-to-text mishearing writes directly into workout history, which then feeds the
 * progression and 1RM engines — so an implausible set must never be committed without the user
 * seeing it. These thresholds and confidence constants are mirrored in
 * `backend/src/lib/voicePlausibility.ts`; change both together or the client and the backend will
 * disagree about the same utterance.
 */

/** Routing gate: at or above this, the local parse is trusted and no request is made. */
